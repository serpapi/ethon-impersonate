...
describe EthonImpersonate::Easy::Options do
  ...
  [
    ...
    describe "#{name}=" do
      ...
      it "sets option" do
        ...
        expect(EthonImpersonate::Curl).to receive(:set_option).with(name, anything, anything)
        value = case name
        when :http_version
          :httpv1_0
        when :httpauth
  ...
  describe "#httppost=" do
    it "raises unless given a FFI::Pointer" do
      expect{ easy.httppost = 1 }.to raise_error(EthonImpersonate::Errors::InvalidValue)
    end
  end
  ...
  context "when requesting" do
    ...
    if EthonImpersonate::Easy.supports_timeout_ms?
