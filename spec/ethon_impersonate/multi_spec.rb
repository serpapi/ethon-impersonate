...
describe EthonImpersonate::Multi do
  describe ".new" do
    ...
    context "with default options" do
      it "allows running #perform with the default execution_mode" do
        EthonImpersonate::Multi.new.perform
      end

      it "refuses to run #socket_action" do
        expect { EthonImpersonate::Multi.new.socket_action }.to raise_error(ArgumentError)
      end
    end
    ...
    context "when options not empty" do
      context "when pipelining is set" do
        ...
        it "sets pipelining" do
          expect_any_instance_of(EthonImpersonate::Multi).to receive(:pipelining=).with(true)
          EthonImpersonate::Multi.new(options)
        end
      end

      context "when execution_mode option is :socket_action" do
        let(:options) { { execution_mode: :socket_action } }
        let(:multi) { EthonImpersonate::Multi.new(options) }
  ...
  describe "#socket_action" do
    ...
    let(:multi) {
      multi = EthonImpersonate::Multi.new(options)
