...
describe EthonImpersonate::Easy do
  ...
  describe ".new" do
    it "inits curl" do
      expect(EthonImpersonate::Curl).to receive(:init)
      easy
    end
    ...
    context "when options are empty" do
      it "sets only callbacks" do
        ...
        expect(EthonImpersonate::Easy).to receive(:set_option).never
        easy
      end
    end
    ...
    context "when options not empty" do
      context "when followlocation is set" do
        ...
        let(:easy) { EthonImpersonate::Easy.new(options) }

        it "sets followlocation" do
          expect_any_instance_of(EthonImpersonate::Easy).to receive(:set_callbacks)
          expect(EthonImpersonate::Curl).to receive(:set_option).with(:followlocation, true, anything)
          easy
        end
      end
    end
    ...
    context "when options aren't empty" do
      ...
      context "when invalid key" do
        it "raises invalid option error" do
          expect{ easy.set_attributes({fubar: 1}) }.to raise_error(EthonImpersonate::Errors::InvalidOption)
        end
      end
    end
  end
  ...
  describe "#reset" do
    ...
    it "resets easy handle" do
      expect(EthonImpersonate::Curl).to receive(:easy_reset)
      easy.reset
    end
  ...
  describe "#mirror" do
    it "returns a Mirror" do
      expect(easy.mirror).to be_a(EthonImpersonate::Easy::Mirror)
    end

    it "builds from easy" do
      expect(EthonImpersonate::Easy::Mirror).to receive(:from_easy).with(easy)
      easy.mirror
    end
  end
