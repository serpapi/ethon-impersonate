...
describe EthonImpersonate::Multi::Options do
  ...
  [
    ...
    describe "#{name}=" do
      ...
      it "sets option" do
        expect(EthonImpersonate::Curl).to receive(:set_option).with(name, anything, anything, anything)
  ...
  context "socket_action mode" do
    let(:multi) { EthonImpersonate::Multi.new(execution_mode: :socket_action) }
