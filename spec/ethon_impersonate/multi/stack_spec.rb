...
describe EthonImpersonate::Multi::Stack do
  ...
  describe "#add" do
    ...
    context "when easy new" do
      it "adds easy to multi" do
        expect(EthonImpersonate::Curl).to receive(:multi_add_handle).and_return(:ok)
        multi.add(easy)
      end
    ...
    context "when multi_add_handle fails" do
      it "raises multi add error" do
        expect(EthonImpersonate::Curl).to receive(:multi_add_handle).and_return(:bad_easy_handle)
        expect{ multi.add(easy) }.to raise_error(EthonImpersonate::Errors::MultiAdd)
      end
    end

    context "when multi cleaned up before" do
      it "raises multi add error" do
        EthonImpersonate::Curl.multi_cleanup(multi.handle)
        expect{ multi.add(easy) }.to raise_error(EthonImpersonate::Errors::MultiAdd)
      end
    end
  end
  ...
  describe "#delete" do
    context "when easy in easy_handles" do
      ...
      it "deletes easy from multi" do
        expect(EthonImpersonate::Curl).to receive(:multi_remove_handle).and_return(:ok)
        multi.delete(easy)
      end
    ...
    context "when multi_remove_handle fails" do
      ...
      it "raises multi remove error" do
        expect(EthonImpersonate::Curl).to receive(:multi_remove_handle).and_return(:bad_easy_handle)
        expect{ multi.delete(easy) }.to raise_error(EthonImpersonate::Errors::MultiRemove)
      end
    end
  end
end
