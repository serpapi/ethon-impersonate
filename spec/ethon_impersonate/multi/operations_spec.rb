...
describe EthonImpersonate::Multi::Operations do
  let(:multi) { EthonImpersonate::Multi.new }
  ...
  describe "#running_count" do
    ...
    context "when hydra has more easys" do
      let(:another_easy) { EthonImpersonate::Easy.new }
  ...
  describe "#get_timeout" do
    context "when code ok" do
      ...
      before do
        expect(EthonImpersonate::Curl).to receive(:multi_timeout).and_return(:ok)
    ...
    context "when code not ok" do
      before { expect(EthonImpersonate::Curl).to receive(:multi_timeout).and_return(:not_ok) }

      it "raises MultiTimeout error" do
        expect{ multi.send(:get_timeout) }.to raise_error(EthonImpersonate::Errors::MultiTimeout)
      end
    end
  end
  ...
  describe "#set_fds" do
    ...
    context "when code ok" do
      before { expect(EthonImpersonate::Curl).to receive(:multi_fdset).and_return(:ok) }
      ...
      context "when max_fd not -1" do
        context "when code smaller zero" do
          before { expect(EthonImpersonate::Curl).to receive(:select).and_return(-1) }

          it "raises Select error" do
            expect{ multi.method(:set_fds).call(timeout) }.to raise_error(EthonImpersonate::Errors::Select)
          end
        end

        context "when code bigger or equal zero" do
          before { expect(EthonImpersonate::Curl).to receive(:select).and_return(0) }
    ...
    context "when code not ok" do
      before { expect(EthonImpersonate::Curl).to receive(:multi_fdset).and_return(:not_ok) }

      it "raises MultiFdset error" do
        expect{ multi.method(:set_fds).call(timeout) }.to raise_error(EthonImpersonate::Errors::MultiFdset)
      end
    end
  end
  ...
  describe "#perform" do
    ...
    context "when four easy handles" do
      let(:easies) do
        ary = []
        4.times do
          ary << another_easy = EthonImpersonate::Easy.new
          another_easy.url = "http://localhost:3001/"
        end
        ary
      end
  ...
  describe "#init_vars" do
    ...
    it "sets @timeval" do
      expect(multi.instance_variable_get(:@timeval)).to be_a(EthonImpersonate::Curl::Timeval)
    end

    it "sets @fd_read" do
      expect(multi.instance_variable_get(:@fd_read)).to be_a(EthonImpersonate::Curl::FDSet)
    end

    it "sets @fd_write" do
      expect(multi.instance_variable_get(:@fd_write)).to be_a(EthonImpersonate::Curl::FDSet)
    end

    it "sets @fd_excep" do
      expect(multi.instance_variable_get(:@fd_excep)).to be_a(EthonImpersonate::Curl::FDSet)
    end
  ...
  describe "#trigger" do
    it "calls multi perform" do
      expect(EthonImpersonate::Curl).to receive(:multi_perform)
      multi.send(:trigger, pointer)
    end
    ...
    it "returns multi perform code" do
      expect(EthonImpersonate::Curl).to receive(:multi_perform).and_return(:ok)
