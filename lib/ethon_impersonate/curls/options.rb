  ...
  module Curls
    ...
    module Options
      ...
      def set_option(option, value, handle, type = :easy)
        ...
        raise NameError, "EthonImpersonate::Curls::Options unknown type #{type}." unless respond_to?(OPTION_STRINGS[type])
        ...
        when :curl_slist
          ...
          raise Errors::InvalidValue.new(option,value) unless value.nil? or value.is_a? FFI::Pointer
        when :buffer
          raise NotImplementedError, "EthonImpersonate::Curls::Options option #{option} buffer type not implemented."
        when :dontuse_object
          raise NotImplementedError, "EthonImpersonate::Curls::Options option #{option} type not implemented."
        when :cbdata
          raise NotImplementedError, "EthonImpersonate::Curls::Options option #{option} callback data type not implemented. Use Ruby closures."
        when :callback
          va_type=:callback
      ...
      def self.option(ftype,name,type,num,opts=nil)
        ...
        when :enum
          if opts.is_a? Array then
            ...
            raise TypeError, "EthonImpersonate::Curls::Options #{ftype} #{name} Expected opts to be an Array or a Hash."
          end
        ...
        when :buffer
          raise TypeError, "EthonImpersonate::Curls::Options #{ftype} #{name} Expected opts to be an Array or a Hash." unless opts.is_a? Integer

        else
          raise ArgumentError, "EthonImpersonate::Curls::Options #{ftype} #{name} Expected no opts." unless opts.nil?
        end
      ...
      def self.option_type(type)
        ...
        define_method(OPTION_STRINGS[type]) do |rt|
          return EthonImpersonate::Curls::Options.const_get(cname).map { |k, v| [k, v[:opt]] } if rt == :enum
          EthonImpersonate::Curls::Options.const_get(cname)
        end
      end
